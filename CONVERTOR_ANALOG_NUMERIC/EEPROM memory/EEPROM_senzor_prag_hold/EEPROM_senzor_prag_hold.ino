...
void afisare_lcd(int counter) {
    ...
    if (counter == 0) {
        ...
        EEPROM_updateAnything(0, bend_prag);
        ...
        EEPROM_updateAnything(4, temp_prag);
        ...
        EEPROM_updateAnything(7, moist_prag);
...
void setup() {
    ...
    initializareADC();
    Serial.begin(9600);
    EEPROM_readAnything(0,bend_prag);
    EEPROM_readAnything(4,temp_prag);
    EEPROM_readAnything(7,moist_prag);
    DDRD &= ~(1 << ButtonPinD);
...
ISR(INT1_vect) {
    if (contor_mode > 0) {
        ...
        contor_mode = N_modes - 1; // Wrap-around to the highest mode
    }
    resetareAD = 0;
    // Debug output to Serial Monitor
    Serial.print("ISR triggered | New Mode: ");
